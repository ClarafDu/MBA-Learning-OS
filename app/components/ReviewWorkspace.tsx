...
const cards = catalog.lectures.flatMap(l=>l.recallQuestions.map(q=>({...q,course:l.course,lecture:l.slug,number:l.number})));
export default function ReviewWorkspace() {
 const {t} = useLanguage();
  ...
  const scoped=cards.filter(c=>(course==='all'||course===c.course)&&(filter==='all'||(filter==='pending'?progress.reviews[c.id]!=='understood':progress.reviews[c.id]===filter)));
  ...
  function mark(status:string) {
    ...
    if (!review(card.id,status)) { setNotice('保存未成功，请检查存储提示。'); setUndo(null); return; }
    setShown(false);
    ...
    <div className="review-layout">
      <section>
        ...
        <T>{!ready ? <div className="panel empty-state" role="status"><T>正在读取学习进度…</T></div> : card ? <article className="study-card">
          <div className="card-meta"><span className="pill"><T>ME · 示例题</T></span><span className="meta"><T>{position+1}</T><T> / </T><T>{scoped.length}</T></span></div>
          ...
          <div className="study-footer"><Link href={'/courses/'+card.course+'/'+card.lecture+'#quick-review'}><T>返回 Lecture </T><T>{String(card.number).padStart(2,'0')}</T></Link><button className="text-button" onClick={()=>{setIndex(position+1);setShown(false);}}><T>下一题 →</T></button></div>
