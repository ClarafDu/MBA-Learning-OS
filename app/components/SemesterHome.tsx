...
export default function SemesterHome(){
 ...
 return <div className="content semester-home compact-home">
  ...
  <div className="two-lines">
   <Link className="product-line schedule-line" href="/calendar"><span className="line-number">01</span><div><p className="eyebrow">TIME & DEADLINES</p><h2>Schedule</h2><p>{en?'Classes, homework deadlines and exam dates. Add them to your own calendar.':'课程日历、作业 Deadline 和考试时间，集中查看并导入个人日历。'}</p><ul><li>{en?'Weekly and monthly views':'本周与月历视图'}</li><li>{en?'Deadline countdowns':'Deadline 倒计时'}</li><li>{en?'Calendar export':'导入手机或电脑日历'}</li></ul></div><b>↗</b></Link>
   <Link className="product-line map-line" href="/map"><span className="line-number">02</span><div><p className="eyebrow">MATERIALS & KNOWLEDGE</p><h2>Map</h2><p>{en?'Navigate courses, find materials and keep private notes in one connected map.':'在可导航的课程地图中查找课件、知识点并记录私人笔记。'}</p><ul><li>{en?'Interactive course map':'可缩放课程地图'}</li><li>{en?'Materials and knowledge search':'课件与知识搜索'}</li><li>{en?'Private notes':'私人笔记'}</li></ul></div><b>↗</b></Link>
  </div>
