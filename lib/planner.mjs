...
export function validateEvent(value) {
 ...
 for(const k of ['description','location','link','materialLink','submissionLink','submissionLabel','publicDetails','detailsLink'])if(value[k]!=null&&(typeof value[k]!=='string'||value[k].length>10000))throw Error('日程文字格式无效 / Invalid event text');
 ...
 for (const key of ['submissionLink','detailsLink']) if(value[key] && !safePublicDestination(value[key])) throw Error('公开任务链接格式无效 / Invalid public task link');
 ...
 return {id:value.id,title:value.title.trim(),course:value.course,kind:value.kind,start:value.start,end:value.end,created:value.created||value.start,visibility:value.visibility,description:value.description||'',location:value.location||'',link:value.link||'',materialLink:value.materialLink||'',submissionLink:value.submissionLink||'',submissionLabel:value.submissionLabel||'',publicDetails:value.publicDetails||'',detailsLink:value.detailsLink||'',reminder:value.reminder||0,done:value.done===true};
}
export function safeUrl(value) {try {const u=new URL(value);return u.protocol==='https:';}catch{return false;}}
export function safePublicDestination(value){if(!value)return true;if(safeUrl(value))return true;return /^mailto:[^@\s]+@[^@\s]+\.[^@\s]+$/.test(value);}
...
export function deadlineUrgency(days){return days<0?'overdue':days<=3?'urgent':days<=7?'warning':'normal';}
